...
export const CreateProductMutation = gql`
    ...
    $categoriesIds: [ID!],
        ...
        createProduct(
            ...
            brand { id, name }
            imageUrl
            productTaxons { id, available, taxon { name } }
        }
    }`;
            ...
            mutate({
                ...
                update: (store, { data: { deleteProductTaxons } }) => {
                    ...
                    data.allProducts = data.allProducts.map((product) => {
                        ...
                        const associatedProduct = product.productTaxons.find(({ id: taxonId }) => (
                            taxonId === deleteProductTaxons.id
                        ));

                        if (associatedProduct) {
                            return {
                                ...product,
                                productTaxons: product.productTaxons.filter(({ id: taxonId }) => (
                                    taxonId !== deleteProductTaxons.id
                                ))
                            }
                        }
